...
<head>
    ...
    <title>Programme détaillé des soutenances par enseignants </title>
</head>
<style>
    ...
    <main>
        ...
        @foreach ($programmation as $enseignant)
            ...
            <table class="content-table" aria-describedby="infos">
                ...
                <tbody>
                    @foreach ($enseignant['programmation'] as $prog)
                        <tr>
                            <td>{{ $enseignant['nom'] }} {{ $enseignant['prenom'] }}</td>
                            <td style="line-height: 2;">
                                Jour: {{ Carbon::parse($prog['jour'])->translatedFormat('d F Y') }}<br>
                                Heure: {{ $prog['heure'] }}<br>
                                Salle: {{ $prog['salle'] }}<br>
                                Rôle: {{ ucfirst($prog['role']) }}
                            </td>
                            <td>{{ $prog['etudiant'] }}</td>
                        </tr>
                    @endforeach
