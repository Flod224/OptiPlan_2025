										...
										<div style="display: flex; justify-content: space-between;">
											...
											@if(session()->has('selected_session_id'))
												<form action="{{ route('afficherPlanning') }}" method="POST" target="_blank">
													@csrf 
													<input type="hidden" name="type" value="Pre-Soutenance">
												...
												<form id="planningForm" action="{{ route('runPlanning') }}" method="POST" target="_blank">
													...
													<button type="button" class="btn btn-success" onclick="handleSubmit()">Programmer la pré-soutenance</button>
												</form>
								...
								<!--begin: Datatable -->
											...
											<table class="m-datatable" id="html_table" width="100%">
												...
												<tbody>
													...
													@if ($etudiant->is_ready >= 0 && $etudiant->is_ready < 4)
														<tr style="text-align: center;">
															...
															<td>
															@if ($etudiant->is_ready == 0)
																	<div style="background-color: red; color: white; width: 100%; font-weight: bold; padding: 5px; border-radius: 5px; text-align: center;">NON<i class="fas fa-info-circle" data-bs-toggle="tooltip" data-bs-placement="top" title="L'étudiant(e) n'est pas prêt(e) pour la pre-soutenance."></i></div>
																@elseif ($etudiant->is_ready == 1)
																	<div style="background-color: orange; color: white; width: 100%; font-weight: bold; padding: 5px; border-radius: 5px; text-align: center;">En Attente<i class="fas fa-info-circle" data-bs-toggle="tooltip" data-bs-placement="top" title="L'étudiant(e) attend une validation."></i></div>
																@elseif($etudiant->is_ready == 2)
																	<div style="background-color: lightgreen; color: white; width: 100%; font-weight: bold; padding: 5px; border-radius: 5px; text-align: center;">Accepté<i class="fas fa-info-circle" data-bs-toggle="tooltip" data-bs-placement="top" title="L'étudiant(e) est prêt pour la pre-soutenance."></i></div>
																@elseif($etudiant->is_ready == 3)
																	<div style="background-color: green; color: white; width: 100%; font-weight: bold; padding: 5px; border-radius: 5px; text-align: center;">Programmé<i class="fas fa-info-circle" data-bs-toggle="tooltip" data-bs-placement="top" title="L'étudiant(e) est programmé(e) pour la pre-soutenance."></i></div>
																@endif
															...
															<td>
																@if($etudiant->is_ready == 1 || $etudiant->is_ready == 3)
																...
																@endif
																@if($etudiant->is_ready == 1 || $etudiant->is_ready == 2 || $etudiant->is_ready == 3)
			...
			$(document).ready(function() {
				...
				$("#toutValiderBtn").click(function(e) {
					...
					Swal.fire({
						...
						if (result.value) {
							updateAllEtudiantsStatus(1,'Pre-Soutenance');
						}
					});
				});
				...
				function updateAllEtudiantsStatus(status, type) {
						$.ajax({
							...
							error: function () {
								Swal.fire('Erreur!', 'Une erreur est survenue lors de la requête.', 'error');
							}
						});
					}
